...
#ifdef __AVX2__
  // Use SIMD only if width >= 8 to avoid gather overhead
  if (width >= 8) {
    // Row-by-row SIMD processing with contiguous memory access
    for (int iy = 0, iyA = yA, iyB = yB; iy < height; ++iy, ++iyA, ++iyB) {
      int ix = 0, ixA = xA, ixB = xB;
      __m256 sum_vec = _mm256_setzero_ps();
      
      // Process 8 pixels at a time within each row (contiguous memory)
      for (; ix + 8 <= width; ix += 8, ixA += 8, ixB += 8) {
        const float* a_ptr = &A(iyA, ixA);
        const float* b_ptr = &B(iyB, ixB);
        
        __m256 a_vec = _mm256_loadu_ps(a_ptr);
      ...
      // Reduce SIMD accumulator for this row
      ...
      // Handle remainder pixels in this row (width % 8)
      for (; ix < width; ++ix, ++ixA, ++ixB) {
        sqrErrSum += agz::math::sqr((A(iyA, ixA) - B(iyB, ixB)));
      }
      
      // Early exit check per row
      if (sqrErrSum > maxSqrErr)
        return sqrErrSum;
    }
    return sqrErrSum;
  }
  // Fall through to scalar code for narrow regions (width < 8)
#endif
  
  // Scalar code for narrow regions or when AVX2 not available
  for (int iy = 0, iyA = yA, iyB = yB; iy < height; ++iy, ++iyA, ++iyB) {
